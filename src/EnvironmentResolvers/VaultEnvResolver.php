...
class VaultEnvResolver extends MultiTenantResolverAbstract implements EnvResolverContract
{
    ...
    /**
     * If true - doesn't fetch all keys at once - but resolve it on demand
     *
     * @var bool
     */
    protected bool $asyncResolver = false;

    /**
     * The secret lifetime in seconds - default 30 seconds
     *
     * @var int
     */
    ...
    {
        if ($this->asyncResolver) {
            Log::warning('Vault sync disabled - async mode');
    ...
    public function resolve(string $appId = ''):? Environment
    {
        ...
        // return pre-loaded applications only
        if (!$this->asyncResolver) {
            return $app;
        }

        if (!$app || $app->isExpired($this->ttl)) {
            if (! ($app = $this->loadSecret($appId))) {
              return null;
            }
            $app->id = $appId;
    ...
    public function loadSecret(string $key = '', $quiet = true): Environment|null
    {
        ...
        Log::debug('Loading secret for Tenant: '. $key);
    ...
    /**
     * On Async mode the tenants resolving on demand - using as a cache of tenants environments
     *
     * @param bool $asyncResolver
     *
     * @return self
     */
    public function setAsyncResolver(bool $asyncResolver): self
    {
